...
export function BatteryCharts({ readings, temperatureUnit }: BatteryChartsProps) {
  ...
  // Apply adaptive sampling for performance optimization
  ...
  const sampledReadings = useMemo(() => {
    return adaptiveSample(readings, {
      ...
      targetPoints: 300  // Downsample to ~300 points for optimal performance
    }, 'timestamp', 'voltage');
  }, [readings]);

  const chartData = sampledReadings.map(reading => ({
    ...reading,
  ...
  // Determine if sampling was applied
  const isSampled = sampledReadings.length < readings.length;
  const samplingInfo = isSampled 
    ? `Showing ${sampledReadings.length} of ${readings.length} data points (optimized for performance)`
    : null;
